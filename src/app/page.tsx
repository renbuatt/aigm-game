...
type Room = { id: string; scenario: Scenario; hostName: string; status: "recruiting" | "playing"; };
type Message = { sender: "player" | "gm"; text: string; type?: "ic" | "ooc" | "system"; };
...
export default function Home() {
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      const replyText = msgType === "ic" ? `AI GM「『${userMsg.text}』ですね。処理します…」` : `AI GM (OOC)「${currentUser.handleName}さん、了解しました: ${userMsg.text}」`;
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 w-full">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleEmailAuth} className="space-y-4">
              ...
              <button type="submit" disabled={authLoading} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition shadow-lg shadow-emerald-900/50 mt-2 disabled:opacity-50">
      ...
      {currentView === "lobby" && currentUser && (
        <div className="flex-1 flex flex-col p-6 max-w-7xl mx-auto w-full overflow-y-auto custom-scrollbar">
          <header className="mb-6 flex justify-between items-end border-b border-slate-700 pb-4">
            <div><h1 className="text-3xl font-extrabold text-emerald-400 mb-1">AI GM MORPG Lobby</h1><p className="text-slate-400 text-sm">遊ぶシナリオとキャラクターを選んで、部屋を立てましょう。</p></div>
                  ...
                  {rooms.map((room) => (
                    <div key={room.id} className="bg-slate-800 border border-slate-700 rounded-xl p-0 hover:border-blue-500 transition relative overflow-hidden group flex">
                      ...
                      <div className="p-4 flex-1">
                        ...
                        <div className="flex gap-2 text-xs text-slate-400 mb-3"><span className="bg-slate-900 px-2 py-0.5 rounded border border-slate-700">システム: {room.scenario.system}</span><span className="bg-slate-900 px-2 py-0.5 rounded border border-slate-700 text-amber-300">ホスト: {room.hostName}</span></div>
                        <p className="text-sm text-slate-300 mb-4 line-clamp-1">{room.scenario.tags}</p>
                        <div className="flex gap-2 items-center">
                          <select className="flex-1 bg-slate-900 border border-slate-700 rounded-lg p-2 text-xs text-white focus:border-blue-500 outline-none" onChange={(e) => { const char = room.scenario.presetCharacters.find(c => c.id === e.target.value); if(char) handleJoinRoom(room, char); }} defaultValue="">
                            <option value="" disabled>参加するキャラクターを選択...</option>
                            {room.scenario.presetCharacters.map(c => <option key={c.id} value={c.id}>{c.name} ({c.job})</option>)}
                          </select>
                        </div>
                      </div>
                    </div>
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex flex-col h-64 shadow-inner">
                ...
                <div className="flex-1 overflow-y-auto space-y-2 mb-3 pr-2 custom-scrollbar">
                  {lobbyMessages.map((msg) => (
                    <div key={msg.id} className="text-sm leading-relaxed"><span className="text-xs text-slate-500">[{msg.time}] </span><span className={`font-bold ${msg.isSystem ? 'text-amber-400' : 'text-emerald-300'}`}>{msg.senderName}: </span><span className="text-slate-200 ml-1">{msg.text}</span></div>
                  ))}
                </div>
                <div className="flex gap-2"><input type="text" value={lobbyInput} onChange={(e) => setLobbyInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSendLobby()} placeholder="メッセージを入力..." className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500" /><button onClick={handleSendLobby} className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg text-sm font-bold transition">送信</button></div>
              </div>
            </div>

            <div className="space-y-6">
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 shadow-lg">
                <div className="flex justify-between items-center mb-3 border-b border-slate-700 pb-2"><h2 className="text-sm font-bold text-blue-400">👤 プレイヤー情報</h2>{!isEditingProfile && (<button onClick={() => { setEditProfileData(currentUser); setIsEditingProfile(true); }} className="text-[10px] bg-slate-700 px-2 py-1 rounded hover:bg-slate-600">編集</button>)}</div>
                {isEditingProfile && editProfileData ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2"><button onClick={() => setIsEditingProfile(false)} className="flex-1 bg-slate-700 text-xs py-2 rounded">キャンセル</button><button onClick={saveProfile} className="flex-1 bg-blue-600 hover:bg-blue-500 font-bold text-xs py-2 rounded">保存</button></div>
                  </div>
                ) : (
                  <div className="flex gap-4 items-start"><img src={currentUser.avatarUrl} alt="Avatar" className="w-16 h-16 rounded-full border-2 border-slate-600 object-cover" /><div className="flex-1"><p className="font-bold text-white text-lg">{currentUser.handleName}</p><p className="text-xs text-slate-400 mt-1 line-clamp-3">{currentUser.bio}</p></div></div>
                )}
              </div>
        ...
        <div className="flex-1 flex flex-col max-w-5xl mx-auto w-full p-4 h-full">
          <header className="bg-slate-800 border border-slate-700 rounded-xl p-3 mb-3 flex justify-between items-center shadow-md">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3"><img src={activeRoom.scenario.imageUrl || NO_IMAGE_SCENARIO} alt="bg" className="w-10 h-10 object-cover rounded border border-slate-600" /><div className="flex flex-col"><span className="text-[10px] text-blue-400 font-bold border border-blue-500/50 bg-blue-900/30 px-2 py-0.5 rounded w-fit mb-0.5">ROOM: {activeRoom.scenario.title}</span><span className="text-sm font-bold text-white flex items-center gap-2"><img src={joinedCharacter.imageUrl || NO_IMAGE_CHAR} alt="me" className="w-5 h-5 object-cover rounded-full" />{joinedCharacter.name}<span className="text-xs font-normal text-slate-400 ml-1">(PL: {currentUser.handleName})</span></span></div></div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => rollDice(joinedCharacter.san, "SAN")} className="bg-cyan-700 hover:bg-cyan-600 text-white text-xs px-2 py-1 rounded font-medium">SAN ({joinedCharacter.san})</button><button onClick={() => rollDice(joinedCharacter.str, "STR")} className="bg-red-700 hover:bg-red-600 text-white text-xs px-2 py-1 rounded font-medium">STR ({joinedCharacter.str})</button><button onClick={() => rollDice(joinedCharacter.dex, "DEX")} className="bg-green-700 hover:bg-green-600 text-white text-xs px-2 py-1 rounded font-medium">DEX ({joinedCharacter.dex})</button><button onClick={() => rollDice(joinedCharacter.int, "INT")} className="bg-blue-700 hover:bg-blue-600 text-white text-xs px-2 py-1 rounded font-medium">INT ({joinedCharacter.int})</button>
            </div>
          </header>
          <div className="flex-1 overflow-y-auto space-y-3 p-4 bg-slate-800/80 rounded-xl border border-slate-700 mb-3 custom-scrollbar">
            {messages.map((msg, index) => (<div key={index} className={`p-3 rounded-xl max-w-[85%] ${msg.sender === "gm" ? "bg-slate-700/90 border-l-4 border-emerald-500 mr-auto text-slate-100" : "bg-blue-600/90 ml-auto text-right text-white"}`}><span className="text-[10px] opacity-60 block mb-1">{msg.sender === "gm" ? "AI GM" : joinedCharacter.name} {msg.type && `[${msg.type.toUpperCase()}]`}</span><p className="text-sm whitespace-pre-wrap leading-relaxed">{msg.text}</p></div>))}
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 flex flex-col gap-2">
            <div className="flex items-center gap-4 text-xs"><label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="msgType" value="ic" checked={msgType === "ic"} onChange={() => setMsgType("ic")} className="accent-blue-500"/><span className="text-blue-400 font-semibold">行動宣言 (IC)</span></label><label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="msgType" value="ooc" checked={msgType === "ooc"} onChange={() => setMsgType("ooc")} className="accent-slate-400"/><span className="text-slate-400">雑談 (OOC)</span></label></div>
            <div className="flex gap-2"><input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSend()} placeholder="行動を入力..." className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500" /><button onClick={handleSend} disabled={isLoading} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2 rounded-lg text-sm font-semibold transition disabled:opacity-50">送信</button></div>
          </div>
        </div>
      )}
    </main>
