...
export default function Home() {
  ...
  const executeAdReward = async () => {
    if (!currentUser) return;
    const td = new Date().toLocaleDateString('ja-JP'); const nc = adViewInfo.date === td ? adViewInfo.count + 1 : 1;
    await supabase.from('profiles').update({ points: (currentUser.points || 0) + 20, ad_view_count: nc, last_ad_view_date: td }).eq('id', currentUser.id);
    setCurrentUser(p => p ? { ...p, points: (p.points || 0) + 20 } : null); setAdViewInfo({ count: nc, date: td });
    alert("20pt 獲得しました！"); setAdModal({ isOpen: false });
  };
  ...
  return (
    <main className="h-screen w-full bg-slate-900 text-slate-100 flex flex-col font-sans overflow-hidden">
      ...
      {currentView === "lobby" && currentUser && (
        <LobbyView currentUser={currentUser} handleLogout={handleLogout} setShowMailbox={setShowMailbox} unreadCount={unreadCount} secretRoomIdSearch={secretRoomIdSearch} setSecretRoomIdSearch={setSecretRoomIdSearch} rooms={rooms} searchedSecretRoom={searchedSecretRoom} setSearchedSecretRoom={setSearchedSecretRoom} executeJoinRoom={executeJoinRoom} availableRooms={availableRooms} spectateRoom={spectateRoom} setEditingScenario={setEditingScenario} setCurrentView={setCurrentView} createdScenarios={createdScenarios} deleteScenario={deleteScenario} openRoomConfigModal={handleOpenRoomConfig} fetchAdminData={fetchAdminData} startTrialPlay={(sc: any) => setAdModal({ isOpen: true, step: 1, scenario: sc, type: 'trial' })} availableScenarios={availableScenarios} openUserProfile={openUserProfile} setScenarioAppealTarget={setScenarioAppealTarget} playArchives={playArchives} setShowTicketModal={setShowTicketModal} exchangeTicketWithPoints={exchangeTicketWithPoints} />
      )}
