...
export default function Home() {
  ...
  const fetchData = async () => {
    ...
    if (scData && scData.length > 0) {
      loadedScenarios = scData.map((d: any) => ({
        ...
        npcList: d.npc_list || "", plot: d.plot || "", 
        prologue: d.prologue || "", epilogue: d.epilogue || "", // ★ 追加
  ...
  const saveScenario = async () => {
    ...
    const dbData = { 
      ...
      npc_list: editingScenario.npcList || "", plot: editingScenario.plot || "", 
      prologue: editingScenario.prologue || "", epilogue: editingScenario.epilogue || "", // ★ 追加
    ...
    try {
      ...
      if (targetTab === "story") {
        roleInstructionLines = [
          ...
          "10. 【プロローグ（導入）について】",
          "セッション開始直後の導入フェーズでは、設定されたプロローグ情報があればそれに従い、無ければ本編プロットから推測して自動生成してください。",
          "【設定されたプロローグ】: " + (activeRoom.scenario?.prologue || "特になし"),
          "この導入部において、事態の把握や最初の試練として【必ずプレイヤー全員が最低1回はダイス判定を行わなければならない状況（判定の要求）】を発生させてください。",
          "11. 【エピローグとエンディング（最重要）】",
          "目的を達成したからといって、いきなり [SCENARIO_END] を出力してゲームを終わらせないでください。",
          "目的達成後は必ず「【エピローグ】」と明記し、事後処理や仲間・NPCとの最後の会話を行うフェーズに入ってください。",
          "その際、設定された【エピローグ情報】があればそれに従い、無ければ物語の結末から自動生成してください。",
          "【設定されたエピローグ情報】: " + (activeRoom.scenario?.epilogue || "特になし"),
          "さらにエピローグ内で、キャラクターの生還の代償や最後の足掻きなど【必ずプレイヤー全員に1回ずつ最後のダイス判定（ロール）を行わせる】状況を作ってください。",
          "PLがエピローグでの行動とダイス判定を十分に終え、物語が完全に着地したと判断できたターンの最後にのみ [SCENARIO_END] を出力してください。",
          "12. 【所持アイテムの自動更新】",
        ...
        if (activeRoom.is_trial) {
          roleInstructionLines.push(
            ...
            "2. 【ダイスロールの強制】短い時間ですが、TRPGの醍醐味を味わってもらうため、必ずセッション内で1回はPLにダイス判定（ロール）を行わせるような状況を作ってください。",
            "3. 【クリフハンガーでの強制終了】ダイスロールの結果が出た直後や、「謎の扉が開き、奥から何かが現れた！」など、一番物語が面白く盛り上がってきた絶頂のタイミングを見計らって、プレイヤーの行動を待たずにバッサリと物語を強制終了させてください。",
            "4. 終了の際は、情景描写の直後に「――この先は本編でお楽しみください！」と期待を煽る文言を入れ、ターンの最後に必ず [SCENARIO_END] を出力してゲームを終わらせてください。",
            ""
          );
        }

        if (isSplitMode && myScene.id !== 'scene_main') {
          roleInstructionLines.push(
          ...
          <div className="bg-slate-800 border border-emerald-700/50 rounded-xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <select value={roomConfigModal.itemVisibility} onChange={(e) => setRoomConfigModal({...roomConfigModal, itemVisibility: e.target.value as any})} className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-sm text-white">
