            ...
            <div className="flex-1 bg-slate-800 border border-slate-700 rounded p-4 overflow-y-auto custom-scrollbar">
              {editingCharIndex !== null && editingScenario.presetCharacters[editingCharIndex] ? (
                <div className="space-y-3">
                  ...
                  <div><label className="text-xs text-slate-400 block mb-1">画像URL</label><input type="text" value={editingScenario.presetCharacters[editingCharIndex].imageUrl} onChange={e=>updateCharacter(editingCharIndex, 'imageUrl', e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-sm" /></div>
