...
export default function GameView({
  ...
  togglePauseRoom, toggleAFK, triggerAutoAction, updateInventory, aiPlayersList, kickUser
}: Props) {
  ...
  const [showStatusModal, setShowStatusModal] = useState(false); // ★ キャラクター紹介用モーダル
        ...
        <div className="absolute inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-indigo-500/50 rounded-xl p-6 w-full max-w-3xl shadow-2xl flex flex-col max-h-[85vh]">
            <div className="flex justify-between items-center border-b border-slate-700 pb-3 mb-4 shrink-0">
              <h3 className="text-xl font-bold text-indigo-400">👥 キャラクター紹介・ステータス</h3>
            ...
            <div className="pt-4 border-t border-slate-700 shrink-0 mt-2">
              <button onClick={() => setShowStatusModal(false)} className="w-full bg-slate-700 hover:bg-slate-600 py-3 rounded text-sm font-bold text-white shadow">閉じる</button>
            </div>
          </div>
        </div>
      )}

      {/* ★ 所持品モーダル（閲覧専用・手動変更不可に変更） */}
      {showInventoryModal && joinedCharacter && (
        <div className="absolute inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-amber-500/50 rounded-xl p-6 w-full max-w-lg shadow-2xl space-y-4 flex flex-col">
            <h3 className="text-xl font-bold text-amber-400 border-b border-slate-700 pb-3">🎒 あなたの所持品</h3>
            <p className="text-xs text-slate-300">
              ※所持品はシステム（GM）が管理しています。新しくアイテムを取得したい場合や、設定上持っているはずの物を追加したい場合は、「GMへ質問」タブから交渉して認められてください。
            </p>
            <textarea 
              readOnly
              value={activeRoom.inventories?.[joinedCharacter.id] || joinedCharacter.items || "特になし"} 
            ...
            />
            <div className="pt-2">
              <button onClick={() => setShowInventoryModal(false)} className="w-full bg-slate-700 hover:bg-slate-600 py-3 rounded text-sm font-bold text-white shadow">閉じる</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-blue-500/50 rounded-xl p-6 w-full max-w-2xl shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
            ...
            <div className="flex gap-3 pt-4 border-t border-slate-700">
              ...
              <button onClick={finishSplitting} disabled={isGeneratingSplit} className="flex-1 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-600 py-3 rounded text-sm font-bold text-white shadow-lg shadow-emerald-900/50">編成を確定して再開</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="bg-slate-800 border border-slate-700 rounded-xl p-3 mb-3 shadow-md flex flex-col gap-2 relative">
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-2">
            <button onClick={leaveGame} className="text-xs bg-slate-700 hover:bg-slate-600 text-white px-3 py-1.5 rounded font-bold shadow">🚪 離脱</button>
          ...
          <div className="flex flex-wrap items-center gap-1 justify-end">
            ...
            {joinedCharacter && (
              <div className="flex gap-1 items-center">
                ...
                {rule === 'storytelling' && (
                  <button onClick={() => rollDice(0, "行動")} className="bg-fuchsia-700 hover:bg-fuchsia-600 text-white text-[10px] px-4 py-1.5 rounded font-bold shadow-lg">🎲 葛藤判定</button>
                )}
              </div>
            )}
                ...
                {(activeRoom.free_image_count || 0) < 3 && (
                  <>
                    <button onClick={() => handleGenerateImage("free")} disabled={isGeneratingImg} className="bg-purple-700 hover:bg-purple-600 disabled:bg-slate-600 text-white text-[10px] font-bold px-3 py-1.5 rounded shadow-lg transition">
                      {isGeneratingImg ? "⏳ 生成中..." : `🖼️ 情景生成(無料残${3 - (activeRoom.free_image_count || 0)})`}
                    </button>
            ...
            <>
              <div className="flex gap-2 border-b border-slate-700 pb-2 items-center overflow-x-auto whitespace-nowrap">
                <button onClick={() => handleTabClick("story")} className={`relative text-xs font-bold px-4 py-2 rounded-t-lg transition ${chatTab === "story" ? "bg-emerald-600/20 text-emerald-400 border-b-2 border-emerald-500" : "text-slate-400 hover:text-white"}`}>
                  📖 {isSplitMode && myScene.id !== 'scene_main' ? 'チーム行動宣言' : '行動宣言 (GMへ)'}
              ...
              <div className="flex gap-2 pt-1">
                ...
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSend()} 
                  placeholder={activeRoom.is_paused ? "中断中のため入力できません" : (chatTab === "story" ? "例：鍵穴を覗き込みます。" : (chatTab === "consult" ? (consultWithAI && !isScenarioEnded ? "例：ねえ、どうしようか？ (AI相棒が返答します)" : "例：PL同士の作戦会議メモ (AIは反応しません)") : "例：今の状況でもう一度目星は振れますか？ 職業柄、〇〇を持っていませんか？"))} 
