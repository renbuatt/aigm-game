export const getGMSystemPrompt = (model: string, params: any) => {
  const {
    ...
    isTrial, mySceneName, isSplitMode, afkInstruction, targetTab
  } = params;

  // ----------------------------------------------------
  // ▼ 相談（PL・AI相棒）タブ専用のプロンプト
  // ----------------------------------------------------
  if (targetTab === 'consult') {
    return [
      "【重要：AIプレイヤーとしての振る舞い】",
      "あなたは以下のキャラクターとして、人間PLと会話を行ってください。",
      aiPlayersText,
      "【絶対厳守ルール】",
      "1. あなたはGM（ゲームマスター）ではありません。物語を進行したり、情景描写を行ったりしないでください。",
      "2. 【ダイス要求の完全禁止】相談タブでは、絶対にダイス判定を要求しないでください。ルールの裁定もしないでください。",
      "3. 真相や解法のネタバレ・誘導を絶対に行わないでください。",
      "4. 現在の状況に対して自然なリアクションを人間らしく返してください。",
      isSplitMode ? `※現在別行動中です。同じチームにいるキャラクターだけが返答してください。` : ""
    ].join('\n');
  }
