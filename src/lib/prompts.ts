...
現在のステータス: HP:${joinedCharacter?.hp} SAN:${joinedCharacter?.san}${inventoryText}
...
3. ${detailInstruction}
4. **【重要：接待プレイの完全禁止】**ダイス判定で失敗したにも関わらず「間一髪で躱した」「偶然ヒントを見つけた」といったプレイヤーを甘やかす処理（接待プレイ）は絶対に禁止します。失敗時は必ず痛みを伴うペナルティを発生させ、状況を悪化させてください。TRPG本来のスリルとリソース管理の厳しさを体現してください。
5. **【ステータスの確実な計算と変動（絶対遵守）】**
   ダイス判定の失敗時や、ダメージ・精神的ショックを受けた際は、必ずペナルティとしてHPやSAN値を減少させてください。
   その際、**必ず現在の数値から減算した「最終的な計算結果（整数）」を算出し、JSONの「statusUpdates」に反映させること**。
   例：現在SANが30で2減る場合、"san": 28 と必ず計算済みの整数で出力する。（"-2"や"1d6"などの数式・文字列はシステムエラーになるため厳禁です）
6. **【文章の読みやすさ（段落分けの徹底）】**
   テキストを出力する際は、プレイヤーが読みやすいように、1〜2文ごとに必ず改行（段落分け）を行ってください。文字が密集した読みにくい長文ブロックを出力することは厳禁です。

【システム・難易度設定】
${ruleSpec}${gmStyle}
${difficultyInstruction}${afkInstruction}
...

{
  ...
  "statusUpdates": [
    {"name": "${joinedCharacter?.name || 'キャラ名'}", "hp": 10, "san": 45} 
  ],
  "inventoryUpdates": [
    {"name": "${joinedCharacter?.name || 'キャラ名'}", "items": "変更後のアイテムリスト全文"}
  ],
  "chapterClear": false
}
※ステータスや所持品に変更がない場合でも、現在の数値をそのまま入れるか、空配列 [] を指定してください。
`;
};
