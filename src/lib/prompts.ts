...
6. **【文章の読みやすさ（改行の徹底）】**テキストを出力する際は、プレイヤーが読みやすいように【1〜2文ごとに必ず改行（JSON内での \\n ）】を入れてください。文字が密集した読みにくい長文ブロックを出力することは厳禁です。
...
{
  "text": "プレイヤーへの返答テキスト（必ず1〜2文ごとに \\n を含めて改行し、読みやすくすること。${targetTab === 'consult' ? volumeInstruction : `【重要】プロローグ・エンディング・章の開始時は絶対に1500〜3000文字で劇的に描写すること。通常の場面は${volumeInstruction}`}）",
  "statusUpdates": [
