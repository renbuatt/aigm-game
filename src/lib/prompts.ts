export const getGMSystemPrompt = (model: string, params: any) => {
  ...
  if (model === 'opus') {
    ...
    detailInstruction = "1回の判定や行動宣言に対して、周囲の微細な変化、五感（温度、湿度、匂い、微かな音、光の陰影）、キャラクターの微細な心理描写、NPCの表情の機微などを、世界最高峰の表現力・語彙力を用いて徹底的かつ圧倒的にリッチに描写してください。絶対に展開を端折らないこと。";
    ...
    detailInstruction = "1回の判定や行動宣言に対して、周囲の状況、五感（視界の暗さ、冷気、音、匂い）、キャラクターの心理描写を深く掘り下げてリッチに描写してください。展開を端折らないこと。";
  } else if (model === 'flash') {
    volumeInstruction = "最低でも1200文字、可能なら1200〜1500文字程度でしっかりと情景を描写すること。";
    detailInstruction = "1回の行動に対して、視覚情報だけでなく聴覚や嗅覚などの五感を交え、状況の変化を詳しく描写してください。急いで解決させないこと。";
