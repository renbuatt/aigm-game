export const getGMSystemPrompt = (model: string, params: any) => {
  ...
  // ★ モデルのランクに応じた描写ボリュームの指定
  let volumeInstruction = "端折らずリッチに600〜1000文字程度、章クリア時は1000〜1500文字程度で描写すること";
  let detailInstruction = "1回の判定や行動宣言に対して、周囲の状況、五感（視界の暗さ、冷気、音、匂い）、心理描写をじっくり丁寧に描写してください。";
  
  if (['pro', 'claude', 'opus'].includes(model)) {
    volumeInstruction = "情景描写と心理描写を極限まで詳細に行い、1000〜2000文字程度の非常に重厚で読み応えのある文章で描写すること。章クリア時は1500〜2500文字程度で劇的に演出すること";
    detailInstruction = "1回の判定や行動宣言に対して、周囲の微細な変化、五感（温度、湿度、匂い、微かな音、光の陰影）、キャラクターの微細な心理描写、NPCの表情の機微などを、あなたの持つ最高の表現力・語彙力を用いて徹底的にリッチに描写してください。絶対に展開を端折らないでください。";
  } else if (model === 'lite' || model === 'flash-lite') {
    volumeInstruction = "400〜800文字程度でテンポ良く描写すること";
    detailInstruction = "周囲の状況や五感を交えつつも、プレイヤーが行動しやすいようにテンポを意識して描写してください。";
  }
